...
class Lingkage_mec_train():
    def __init__(self, crank_location, status_location, epochs=10000, lr=0.01, lr_min=0.0001, gamma=1.00,trajectory_type = 'linear', trajectory_data = [(-3, 0), (3, 0)], device = 'cpu',visualize_mec=False):
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            if not is_lr_min and loss.item() < 50:
                print(f"Reducing LR to {self.lr_min} as loss is below 50.")
                for param_group in self.optimizer.param_groups:
                    param_group['lr'] = self.lr_min
                is_lr_min = True
            elif is_lr_min and loss.item() > 50:
                print(f"Increasing LR to {self.lr} as loss is above 50.")
                for param_group in self.optimizer.param_groups:
